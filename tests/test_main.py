...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["process-images", "{temp_dir}", "test", "--padding", "4"], 0, "Processing images..."),
        (["process-images", "{temp_dir}", "test", "--trigger", "watercolor"], 0, "Processing images..."),
        (["process-images", "{temp_dir}", "test", "--is-anime"], 0, "Processing images..."),
        (["process-images", "{temp_dir}", "test", "--is-object"], 0, "Processing images..."),
...
def test_commands(runner, temp_dir, command, expected_exit_code, expected_output):
    command = [arg.format(temp_dir=temp_dir) if isinstance(arg, str) else arg for arg in command]
...
def test_process_images_output(runner, temp_dir):
    ...
    name = "test"
    padding = 4
    sizes = [512, 1024, 672]  # Include 672 as it's used in the process_directory function
    num_images = 3
    ...
    for i in range(1, num_images + 1):
        ...
        img_array = np.random.rand(100, 100, 3) * 255
        img = Image.fromarray(img_array.astype('uint8'))
        original_filename = f"image_{i-1:04d}.jpg"  # Match the naming in process_directory
        img.save(os.path.join(temp_dir, original_filename))

    # Run the process-images command
    result = runner.invoke(ecko_cli, ["process-images", temp_dir, name], catch_exceptions=False)
    print(f"Exit code: {result.exit_code}")
    print(f"Output:\n{result.output}")
    ...
    for i in range(1, num_images + 1):
        ...
        for size in sizes:
            ...
            if size == 672:
                assert not os.path.exists(expected_file), f"Unexpected 672 file found: {expected_file}"
            else:
        ...
        assert os.path.exists(expected_txt), f"Expected text file not found: {expected_txt}"
